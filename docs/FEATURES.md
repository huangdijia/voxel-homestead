...
- M1 基线为 134 项通过、1 项跳过；900 秒性能数据不包含后来的农牧增量。最新全量为 **286 项通过、1 项跳过，类型检查与生产构建通过**；农业 9 阶段、旧档升级、Chrome 全屏／安装入口／离线保存的指定场景分别见 [M2 验证](M2-VALIDATION.md) 与 [网页应用验证](WEB-APP-VALIDATION.md)。不代表全部浏览器、自然生存旅程或整个 M2 已验收。
...
- 当前写入存档版本 2，读取时仍兼容版本 1；首次升级保存与保留旧检查点在同一 IndexedDB 事务内完成。旧世界保留生成器版本 1，新世界使用生成器版本 2；未知未来版本被拒绝。世界列表提供升级前备份导出；迁移故障和备份守恒有专用用例，Chrome 旧档升级、备份下载及独立恢复已通过，详见 [M2 验证](M2-VALIDATION.md)。

## M2：完整主世界生存

本轮实现 **农业与猪羊养殖子集**，不代表 M2 完成。[browser-agriculture.mjs](../scripts/browser-agriculture.mjs) 已在 Chrome 实际运行，9 阶段通过、4 次 IndexedDB 检查点回读一致，结果与限制已归档至 [M2 验证](M2-VALIDATION.md)。脚本先检查空白生存世界，明确记录工具／原料／动物／平台夹具，通过业务断言后才追加展示农田，不能等同于无辅助的手动生存旅程或全部浏览器验收。
