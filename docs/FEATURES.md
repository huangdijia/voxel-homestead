...
## 海边别墅与枪械增量（2026-09-05）

- 生成器 7：出生点旁的海湾、沙滩、棕榈、两层白色别墅（入口 X 32 / Y 69 / Z −10）、室内家具、楼梯、海景露台、可游泳并退出的泳池、栈桥。所有结构均为可编辑的体素。新建世界生效，旧生成器 1–6 不变。
- 海岸步枪：新旧世界、生存与创造模式均可按 G 领取／装备；背包满时明确提示，不覆盖物品。长按左键连射，最大射程 72 格、单发伤害 8、间隔不低于 0.12 秒；无限弹药、无耐久、无需换弹。
- 弹道按最近实体包围盒命中，实体之前的固体／玻璃和未加载区块挡弹；不破坏建筑、不触发挖矿，击败生物沿用经验与掉落。配有第一人称枪模、枪口闪光、弹道、命中准星、合成枪声和中文弹药提示。
- 枪械为本游戏新增玩法，不属于 Minecraft Java 原版功能。目前为一种步枪，暂无瞄准镜、多人或实体抛射弹道。
- 规则覆盖见 `tests/coast.test.ts` 与 `tests/firearms.test.ts`，真实浏览器范围见 [验证记录](VALIDATION.md)。
...
- M1 基线为 134 项通过、1 项跳过；900 秒性能数据不包含后来的农牧与自然更新。自然更新当时全量为 **406 项通过、1 项可选 CPU 基准跳过，类型检查与生产构建通过**。上一轮农业 9 阶段、旧档升级、Chrome 全屏／安装入口／离线保存的指定场景见 [M2 验证](M2-VALIDATION.md) 与 [网页应用验证](WEB-APP-VALIDATION.md)，不自动外推到本轮版本。本轮 [Chrome 开发版自然更新记录](browser-natural.json) 为 7 阶段通过、3 次检查点比较一致，推进 41.5 个模拟秒；使用创造模式和显式夹具，已通过页面重载比较并保留真实截图。不代表全部浏览器、自然生存旅程、持续运行性能或整个 M2 已验收。

## 浏览器应用增量

| ID | 功能 | 实现与验收 |
| --- | --- | --- |
| WEB-FULLSCREEN-01 | 点击进入／继续时请求全屏，Esc 暂停并退出；失败允许窗口游玩 | 已实现；10 项事件回归，Chrome 生产版全屏 + Pointer Lock、E 背包、原生 Esc 实测通过。其他浏览器同路径待完整验证。 |
| WEB-PWA-01 | 安装入口、应用图标、离线资源缓存与安全更新 | 已实现；7 项事件回归、生产缓存检查、Chrome 原生安装窗口和离线刷新／世界加载／保存回读通过。实际安装步骤在确认窗口取消，未宣称桌面应用完整验收。 |
...
| M1-WATER-01 | M1 | 水中游泳、上浮、氧气恢复与溺水 | 已实现／待浏览器全面验收 | [engine.test.ts](../tests/engine.test.ts)：`distinguishes closed doors and open leaves and ignores water collision`；[survival-systems.test.ts](../tests/survival-systems.test.ts)：`water, breathing and swimming`，覆盖氧气耗尽前免伤、周期溺水、跳跃上浮、浅水呼吸、出水恢复及创造免罚 | 入水、持续潜水耗尽氧气、受到伤害、浮出水面恢复；氧气 HUD 与保存重进后的状态一致。传播及流面高度的新增规则见 M2-NATURE-01，M1 的历史游泳证据不替代其验收。 |
...
- 建造高度 −64…319，新世界采用生成器 5；三维视距窗口随玩家上下移动，视距 6 上限 2,197 个子区块。旧生成器 1–4 地形保持；修改数据按玩家改动规模增长。见 [完整高度记录](WORLD-HEIGHT.md)。
- 水／熔岩已支持向下及水平传播、撤源退流与接触转化；水源可有条件再生。水浸方块、流体推力、熔岩引燃及离开熔岩后的持续燃烧尚未实现。沙子／沙砾按完整体素逐格下落，尚无平滑下落实体或砸伤效果。
- 门、梯子、床固定朝向；火把支持对方块表面放置，但墙面火把暂使用同一竖直模型。
- 原生树叶支持连接判定、随机衰减及掉落，玩家叶永久保留；橡树苗可自然或用骨粉长成固定树形，仍不是完整树木生态。猪羊已支持喂食繁殖、幼体成长和剪毛产出，其他生物的繁殖、驯养及坐骑仍待实现；生物寻路为简单避碰／跳障碍。
- 光照是天空竖直遮挡加近处点光，不是原版完整方块光传播；方块光强估计可能穿墙。天气仍未实现。
- 饥饿与护甲遵循首期简化规则，尚未包含饱和度、所有伤害来源与全部数值。
- 只支持生存普通难度和创造；先支持电脑键鼠，移动端展示菜单但不提供触控游玩。
- 当前写入存档版本 5，读取兼容版本 1–5。首次升级保存与保留对应版本旧检查点在同一 IndexedDB 事务内完成，旧世界保留生成器 1／2／3／4，新世界使用生成器 5；保存格式升级不会替换旧地形。流体与自然队列仍完整保存。历史自然更新证据见 [自然更新说明](NATURAL-UPDATES.md)，矿物版本 3→4 见 [矿物记录](MINERALS.md)，本轮高度与迁移见 [高度记录](WORLD-HEIGHT.md)。

## M2：完整主世界生存

当前实现 **农业、猪羊养殖与自然更新子集**，不代表 M2 完成。上一轮 [browser-agriculture.mjs](../scripts/browser-agriculture.mjs) 已在 Chrome 实际运行，9 阶段通过、4 次 IndexedDB 检查点回读一致，结果与限制已归档至 [M2 验证](M2-VALIDATION.md)。本轮自然更新的 7 阶段开发版脚本与 3 次检查点结果见 [原始记录](browser-natural.json)，页面重载比较与真实截图已完成。两类脚本都使用明确记录的输入和展示夹具，不能等同于无辅助的手动生存旅程或全部浏览器验收。
...
| M2-WATER-01 | M2 | 铁桶收取／放置水源，堆叠空桶转换为水桶，再用于灌溉 | 已实现／待浏览器全面验收 | [agriculture-integration.test.ts](../tests/agriculture-integration.test.ts)：`transfers a source into a stacked bucket and places it without creating extra buckets`、`blocks water collection through a wall and preserves buckets in unloaded land`；[natural-integration.test.ts](../tests/natural-integration.test.ts)：加载边界前的水源回收与桶守恒；流动水灌溉实现见 [farming.ts](../src/game/farming.ts) | 真实右键搬运水源，灌溉 4 格范围耕地；确认桶数量守恒、墙后及未加载水源不可操作。桶仅回收源方块；流动水也可参与灌溉，传播、撤源和双源再生规则见 M2-NATURE-01，该项不关闭整个自然更新域。 |
...
| --- | --- | --- | --- |
| M2-MINERALS-01 | M2 | 铜、金、红石、青金石、钻石、绿宝石、深层矿石与深板岩 | 生成器 4 的历史压缩分布见 [矿物记录](MINERALS.md)，生成器 5 的完整高度偏好见 [高度记录](WORLD-HEIGHT.md)。仍未对齐原版群系限制与矿脉密度。 |
| M2-HEIGHT-01 | M2 | −64…319 建造范围、深洞与高山、三维区块窗口、未加载边界、版本 5 存档 | 有独立生成／规则／流式加载／迁移测试及专用浏览器夹具，详见 [高度记录](WORLD-HEIGHT.md)。旧生成器 1–4 保持，当前高度范围一致不表示地形算法或完整群系已一致；新版长时性能和跨浏览器仍需独立验收。 |
| M2-EQUIPMENT-01 | M2 | 金／钻石工具与护甲，独立挖掘速度、采集等级和韧性 | 天然钻石→钻石镐→黑曜石以及配方／耐久／死亡拾回由专属测试与浏览器记录验收；首批附魔见下表，攻击边缘规则、全部装备材质仍待完成。 |
| M2-MATERIALS-01 | M2 | 粗铜／粗金熔炼、材料压块拆回、粒锭互换 | 校验材料扣除、满背包失败、熔炼中保存和恢复；红石块没有电路行为，铜没有氧化，绿宝石没有交易功能。 |

### 经验与首批附魔增量

本轮全量 874 项通过、1 项可选基准跳过；类型检查与生产构建通过。详细玩法、来源和验收记录见 [经验与附魔](ENCHANTING.md)。八种附魔的局部实现不关闭 42 种附魔的完整目标。

| ID | 阶段 | 局部实现 | 验收与剩余边界 |
| --- | --- | --- | --- |
| M2-XP-01 | M2 | 采矿、击杀、繁殖、炉产物经验，经验球拾取、等级消费、死亡返还 | experience/enchanting-integration 测试覆盖等级边界、一次领取、未加载暂停、容量与保存；完整经验生态仍有缺口。 |
| M2-ENCHANT-01 | M2 | 附魔台、15书架加成、三档报价、稳定种子，八种装备附魔 | enchanting/enchantments/enchanting-integration/visuals 测试验证扣料、效果和保存。时运首期仅矿物；附魔书、铁砧与砂轮的新增实现见下表，其余 34 种附魔待实现。 |
| M2-BOOK-01 | M2 | 牛、皮革与牛肉，甘蔗、纸、书、书架和附魔台的材料链 | sugar-cane/enchanting-resources 测试覆盖生长支撑、配方、牛掉落／繁殖；天然甘蔗仅新增生成器6。旧1–5地形不变。 |
| M2-SAVE6-01 | M2 | 保存经验球／点数／种子、物品附魔、炉内经验与甘蔗任务 | 严格校验版本边界与元数据；旧档升级事务保留完整备份。正常退出和临时附魔槽位守恒有集成用例。 |

### 铁砧、砂轮与附魔书增量

规则、材料来源、存档 v7 和验收证据见 [铁与旧书](WORKSHOPS.md)。生成器保持 6，八种附魔之外的效果与诅咒仍待实现。

| ID | 阶段 | 局部实现 | 验收与边界 |
| --- | --- | --- | --- |
| M2-WORKSHOP-01 | M2 | 铁砧材料修理、同类装备合并、附魔书转移与升级、命名、历次加工成本、使用磨损 | workshops/workshop-integration/workshop-placement 测试覆盖费用、元数据、取出原子性、创造免经验及三档状态；附魔池不完整。 |
| M2-WORKSHOP-02 | M2 | 砂轮修复、移除当前附魔、经验返还、附魔书还原普通书 | 输出只在成功取出时消费输入并发经验；未来诅咒需独立保留规则。 |
| M2-BOOK-02 | M2 | 普通书附魔、跨装备类别的附魔书、8 种创造模式书 | 表内保持当前 8 种实现池；不等于原版全量附魔书获取渠道和概率。 |
| M2-SAVE7-01 | M2 | 名称、加工成本、附魔书、命名容器与下落铁砧距离保存 | v6→7 原子备份、旧版本隔离和中断回滚有测试；旧生成器及地形不变。 |
| M2-ANVIL-FALL-01 | M2 | 有界铁砧下落、落地伤害／磨损、头盔减伤、堆叠支撑 | 沿用格子步进重力；不宣称平滑落体、流体推力及所有局部形状着陆行为已对齐。 |
| M2-STONE-SLAB-01 | M2 | 石台阶上下半格、合并双层、挖掘掉落与砂轮配方 | 实际射线放置集成测试；并非全部材质台阶和水浸行为。 |

### M2 尚未完成的功能域

| ID | 阶段 | 功能域／后续内容 | 状态 | 验收目标 |
| --- | --- | --- | --- | --- |
| M2-CATALOG-01 | M2 | 主世界群系、结构、植物、方块、矿物与装备内容扩展 | 部分已实现／待浏览器全面验收 | 已有独立内容审计基线、农牧／自然更新／矿物和高度子集；继续补齐缺域、自然生成／获取链、交互与存档用例。钻石镐采集黑曜石已有独立证据，传送门链尚未实现；审计清单存在也不表示主世界内容扩展完成。 |
| M2-ECOLOGY-01 | M2 | 更多生物、农业、生长、繁殖、驯养和坐骑 | 部分已实现／待浏览器全面验收 | 当前为四类作物、猪羊牛、甘蔗及固定橡树生长子集，详见上表与 M2-NATURE-01；继续打通甜菜自然来源、其余植物、生物、驯养／坐骑及跨区块生命周期。 |
| M2-VILLAGE-01 | M2 | 村民、职业与交易 | 待实现 | 建立完整交易条件、商品和价格状态、补货与限制；消费和产出守恒，离开区域及重载后状态一致。 |
| M2-PROGRESS-01 | M2 | 钻石等装备、经验、附魔、酿造和钓鱼 | 部分已实现／待浏览器全面验收 | 已有矿物装备及下列经验／八附魔子集；酿造、钓鱼、其余附魔仍未实现。 对每条成长链验证材料来源、配方／操作、效果、耐久或消耗、经验变化和存档；不能只验证物品目录。 |
| M2-NATURE-01 | M2 | 天气、动态流体、重力方块、树叶更新与完整照明 | 部分已实现／待浏览器全面验收 | [自然更新说明](NATURAL-UPDATES.md) 列出规则与限制；[fluids.test.ts](../tests/fluids.test.ts)、[natural-updates.test.ts](../tests/natural-updates.test.ts)、[natural-integration.test.ts](../tests/natural-integration.test.ts) 与 [natural-visuals.test.ts](../tests/natural-visuals.test.ts) 覆盖传播／撤源／混合、掉落守恒、加载边界、树苗条件、存档和几何。本轮 Chrome 开发版 7 阶段通过、3 次检查点比较一致；截图及页面重载已完成。**天气、完整方块光传播、水浸方块、流体推力、熔岩引燃与持续燃烧、平滑落体及砸伤仍未实现**；还需跨浏览器和长时资源／帧率验收，不复用 M1 性能结论。 |
